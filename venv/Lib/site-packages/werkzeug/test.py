...
from .datastructures import auth
...
class EnvironBuilder:
    ...
    :param auth: An auth object to use for the
        ``auth`` header value. A ``(username, password)`` tuple
        is a shortcut for ``Basic`` auth.
    ...
    def __init__(
        ...
        if auth is not None:
            ...
            self.headers.set("auth", auth.to_header())
